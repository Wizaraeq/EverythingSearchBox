...
{
	[Plugin( PluginType.Static, Author = "Quizo", Name = "日本語文字コード判定", Version = "1.0.0.0",
			 Description = "以前のバージョンのQTTabBarで使用していた、G-PROJECTさん作の日本語文字コード判別クラスTxtEncを利用したプラグインです。日本語に関して以前と同様の判定結果が得られます。" )]
	public class JapaneseEncodingDetector : IEncodingDetector
	{
		...
		public Encoding GetEncoding( ref byte[] data )
		{
			var enc = TxtEnc.GetEncoding( ref data );
			if( enc != null )
			{
				return enc;
			}
	...
	/// <summary>
	/// 日本語の文字コード判別クラス
	/// このクラスはG-PROJECTさんということろのものを改変して使わせてもらっています。
	/// だけどなんかサイトがなくなっちゃいましたね
	/// http://www.gprj.net/
	/// </summary>
	static class TxtEnc
	{
		...
		//ExCode一覧
		private const int CDEX_FLG_ERROR = 1;//エラーなもので読み込むためにはFIX必要
		private const int CDEX_FLG_FIX = 2;//仕様上正しいが読み込むためにはFIX必要
		private const int CDEX_FLG_INFO = 4;//情報

		//UTF-7
		private const int CDEX_UTF_7_ERROR_BASE64 = CDEX_FLG_ERROR + 32;
		private const int CDEX_UTF_7_FIX_RFC2060 = CDEX_FLG_FIX + 64;
		//UTF-8
		private const int CDEX_UTF_8_ERROR_DOUBLE = CDEX_FLG_ERROR + 32;
		private const int CDEX_UTF_8_ERROR_0XC0 = CDEX_FLG_ERROR + 64;
		private const int CDEX_UTF_8_INFO_BOM = CDEX_FLG_INFO + 128;
		private const int CDEX_UTF_8_INFO_UCS4 = CDEX_FLG_INFO + 256;
		//UTF-16/32	
		private const int CDEX_UTF_INFO_BOM_BE = CDEX_FLG_INFO + 32;
		private const int CDEX_UTF_INFO_BOM_LE = CDEX_FLG_INFO + 64;
		//ISO2022JP
		private const int CDEX_ISO2022_INFO_ESC = CDEX_FLG_INFO + 32;
		private const int CDEX_ISO2022_INFO_SISO = CDEX_FLG_INFO + 64;
		private const int CDEX_ISO2022_INFO_JIS0208_1978 = CDEX_FLG_INFO + 128;
		private const int CDEX_ISO2022_INFO_JIS0208_1983 = CDEX_FLG_INFO + 256;
		private const int CDEX_ISO2022_INFO_JIS0208_1990_1 = CDEX_FLG_INFO + 512;
		private const int CDEX_ISO2022_INFO_JIS0213_2000_A1 = CDEX_FLG_INFO + 1024;
		private const int CDEX_ISO2022_INFO_JIS0213_2000_A2 = CDEX_FLG_INFO + 2048;
		private const int CDEX_ISO2022_INFO_JIS0201_1976_K = CDEX_FLG_INFO + 4096;
		private const int CDEX_ISO2022_INFO_ASCII = CDEX_FLG_INFO + 8192;
		private const int CDEX_ISO2022_INFO_JIS0201_1976_RS1 = CDEX_FLG_INFO + 16384;
		private const int CDEX_ISO2022_INFO_JIS0201_1976_RS2 = CDEX_FLG_INFO + 32768;
		private const int CDEX_ISO2022_INFO_JIS0208_1990_2 = CDEX_FLG_INFO + 65536;
		//EUC
		private const int CDEX_EUC_INFO_EXK = CDEX_FLG_INFO + 32;

		private static string srcCodec;
		//private static byte[] srcByte;
		private static int srcMaxRead = 2048;
		...
		{//判別
			...
			//UTF-16/32
			if( Chk_UTF_16_32( ref txt, ref RP, AS, ref code, ref excode, ref code2, ref code3 ) == 1 )
			{
				...
				{//判定メインループ jis sjis euc utf7 utf8
					...
					{//ISO2022
						//**DBG-OK
						rcode = Chk_JPN_ISO2022( ref txt, ref RP, AS, ref code, ref excode, ref code2, ref code3, ref code4 );
					}
					else if( txt[RP] > 0x7F )
					{
						if( txt[RP] > 0xBF && txt[RP] < 0xFE && code4[CD_UTF_8] == false )//@@@@@ && code4[CD_UTF_8]==false追加
						{//UTF
							switch( Chk_UTF_8( ref txt, ref RP, AS, ref code, ref excode, ref code2, ref code3, ref code4 ) )
							{
								case 0:
									//ﾌﾞｯﾁｬｹ ｱﾘｴﾅｲ
									rcode = 0;
									break;
								case 1:
									rcode = Chk_JPN_SJIS_EUC( ref txt, ref RP, AS, ref code, ref excode, ref code2, ref code3, ref code4 );
									break;
								case 2:
									rcode = 2;
									break;

								// Edited by Quizo, force to return UTF8 when BOM exists.
								case 3:
									rcode = 2;
									code2 = CD_UTF_8;
									goto exitfor;
					...
					{
						rcode = Chk_UTF_7( ref txt, ref RP, AS, ref code, ref excode, ref code2, ref code4 );
					}
					switch( rcode )
					{
						case 0:
		...
		private static int Chk_UTF_7( ref byte[] txt, ref int RP, int AS, ref int[] code, ref int[] excode, ref int code2, ref bool[] code4 )//@@@@add code4
		{//UTF-7判別
			/*
			UTF-7
			Index		CD_UTF_7	(6)
			...
			*/
			if( !( code2 == -1 || code2 == CD_UTF_7 ) )
			{
				return 1;
			}//確定コード回避
			if( code4[CD_UTF_7] == true )
			{
				return 1;
			}//除外コード回避
			int i;
		...
		{//UTF-8判別
			...
			*/
			if( !( code2 == -1 || code2 == CD_UTF_8 ) )
			{
				return 1;
			}//確定コード回避
			if( code4[CD_UTF_8] == true )
			{
				return 1;
			}//除外コード回避
			int size = 0,
		...
		//RS, code4
		private static int Chk_UTF_16_32( ref byte[] txt, ref int RP, int AS, ref int[] code, ref int[] excode, ref int code2, ref int code3 )//@@@@add code4
		{//UTF-16/32判別
			/*
			UTF-16
			Index		CD_UTF_16	(4)
		...
		private static int Chk_JPN_ISO2022( ref byte[] txt, ref int RP, int AS, ref int[] code, ref int[] excode, ref int code2, ref int code3, ref bool[] code4 )
		{
			...
			*/
			...
			if( !( code2 == -1 || code2 == CD_JPN_ISO2022 ) )
			{
				return 1;
			}//確定コード回避

			if( code4[CD_JPN_ISO2022] == true )
			{
				return 1;
			}//除外コード回避
		...
		private static int Chk_JPN_SJIS_EUC( ref byte[] txt, ref int RP, int AS, ref int[] code, ref int[] excode, ref int code2, ref int code3, ref bool[] code4 )
		{
			...
			*/
			if( !( code2 == -1 || code2 == CD_JPN_SJIS || code2 == CD_JPN_EUC ) )
			{
				return 1;
			}//確定コード回避

			if( code4[CD_JPN_EUC] == true && code4[CD_JPN_SJIS] == true )
			{
				return 1;
			}//除外コード回避
